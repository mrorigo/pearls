...
fn test_io_error_formatting() {
    ...
    assert!(msg.contains("IO error"), "IO error should contain 'IO error' prefix");
}
...
fn test_json_error_formatting() {
    ...
    let msg = error.to_string();
    assert!(msg.contains("JSON error"), "JSON error should contain 'JSON error' prefix");
}
...
fn test_invalid_pearl_error_formatting() {
    ...
    assert!(msg.contains("Title cannot be empty"), "Should preserve context message");
}
...
fn test_not_found_error_formatting() {
    ...
    assert!(msg.contains("prl-a1b2c3"), "Should include Pearl ID in message");
}

#[test]
fn test_cycle_detected_error_formatting() {
    let cycle = vec!["prl-a1b2c3".to_string(), "prl-d4e5f6".to_string(), "prl-a1b2c3".to_string()];
    let error = Error::CycleDetected(cycle.clone());
    let msg = error.to_string();
    assert!(msg.contains("Cycle detected"), "Should contain 'Cycle detected' prefix");
...
fn test_invalid_transition_error_formatting() {
    ...
    assert!(msg.contains("Cannot transition"), "Should explain why transition is invalid");
    assert!(msg.contains("blocked"), "Should mention blocking constraint");
}
...
fn test_ambiguous_id_error_formatting() {
    ...
    let msg = error.to_string();
    assert!(msg.contains("Ambiguous ID"), "Should contain 'Ambiguous ID' prefix");
...
fn test_git_error_formatting() {
    ...
    assert!(msg.contains("Failed to push"), "Should preserve Git error context");
}
...
fn test_error_debug_formatting() {
    ...
    assert!(debug_msg.contains("NotFound"), "Debug format should show variant name");
    assert!(debug_msg.contains("prl-test"), "Debug format should show context");
}
...
fn test_error_context_preservation_in_invalid_pearl() {
    ...
    let msg = error.to_string();
    assert!(msg.contains(context), "Error should preserve full context message");
}
...
fn test_error_context_preservation_in_cycle() {
    ...
    let msg = error.to_string();
    for id in &cycle {
        assert!(msg.contains(id), "Cycle message should contain all node IDs");
    }
}
...
fn test_error_context_preservation_in_ambiguous_id() {
    let partial = "abc";
    let matches = vec!["prl-abc123".to_string(), "prl-abc456".to_string(), "prl-abc789".to_string()];
...
fn test_agent_friendly_error_messages() {
    ...
    assert!(msg.contains(":"), "Error message should have structured format");
...
fn test_error_message_consistency() {
    ...
    assert_eq!(error1.to_string(), error2.to_string(), "Same error should produce same message");
}
...
fn test_error_message_distinguishability() {
    ...
    assert_ne!(error1.to_string(), error2.to_string(), "Different errors should produce different messages");
}
